...
// Better long-term: inject this with wp_localize_script(), but leaving as-is for now.
...
function ajaxDelete(scoreId) {
  ...
  jQuery
    .post(GOLF_AJAX_URL, { action: "golf_final_action_delete", score_id: scoreId }, null, "json")
    .done(function (res) {
...
function ajaxUpdate(scoreId) {
  ...
  // Your button has class "btn-save" so this is OK as long as it's inside the row.
  ...
  jQuery
    .post(GOLF_AJAX_URL, data, null, "json")
    .done(function (res) {
      console.log("SAVE response:", res);

      if (!res || res.success !== true) {
        alert((res && res.data && res.data.message) ? res.data.message : "Update failed. See console.");
...
function golfSaveAll() {
  ...
  jQuery
    .post(GOLF_AJAX_URL, { action: "golf_final_action_bulk_save", rounds: rounds }, null, "json")
    .done(function (res) {
